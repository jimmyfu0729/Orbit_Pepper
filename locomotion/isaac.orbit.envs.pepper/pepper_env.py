...
from omni.isaac.orbit_envs.locomotion.velocity import PepperEnvCfg, RandomizationCfg
...
class PepperEnv(IsaacEnv):
    ...
    def __init__(self, cfg: PepperEnvCfg, headless: bool = False, viewport: bool = False, **kwargs):
        ...
        self.action_space = gym.spaces.Box(low=-10.0, high=10.0, shape=(self.num_actions,))
    ...
    def _design_scene(self) -> List[str]:
        ...
        # object
        self.object.spawn(self.template_env_ns + "/Object", translation=(5.0, 0.0, 0.0))
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        # calculate the position distances btw target and the current position of the robot.
        #position_err=np.linalg.norm(self.actions[:, :2] - self.robot.data.root_pose_w[:, :2])
        #position_x_err = self.actions[:, :1] - self.robot.data.root_pose_w[:, :1]
        #position_y_err = self.actions[:, :2] - self.robot.data.root_pose_w[:, :2]
        #self.actions[:, :0] = position_x_err
        #self.actions[:, :2] = np.arcsin(position_y_err/ position_err)
        #self.robot_actions[:, :self.robot.base_num_dof] = self.robot.data.base_dof_vel[:, :self.base_num_dof]
        ...
        position_err=torch.norm(self.actions[:, :2] - self.robot.data.base_dof_pos[:, :2],dim=1)
        #object_position_error = torch.norm(self.object.data.root_pos_w - self.object_des_pose_w[:, 0:3], dim=1)
        self.extras["is_success"] = torch.where(position_err < 0.02, 1, self.reset_buf)
    ...
    def _pre_process_cfg(self) -> None:
        ...
        self.cfg.robot.rigid_props.disable_gravity = False
    ...
    def _debug_vis(self):
        ...
        # -- goal
        self._goal_markers.set_world_poses(self.object_root_pose[:, 0:3])
        # -- end-effector
        self._ee_markers.set_world_poses(self.robot.data.base_dof_pos[:, 0:3])
        # -- task-space commands
        '''if self.cfg.control.control_type == "inverse_kinematics":
    ...
    def _check_termination(self) -> None:
        ...
        if self.cfg.terminations.is_success:
            position_error = torch.norm(self.object.data.root_pos_w - self.robot.data.base_dof_pos[:, 0:3], dim=1)
            self.reset_buf = torch.where(position_error < 0.02, 1, self.reset_buf)
...
class MoveObservationManager(ObservationManager):
    ...
    #def arm_dof_pos_scaled(self, env: PepperEnv):
    ...
    def base_dof_vel(self, env: PepperEnv):
        """DOF velocity of the arm."""
        return env.robot.data.base_dof_vel
    
    #def tool_dof_pos_scaled(self, env: PepperEnv):
    ...
    #def object_relative_tool_positions(self, env: PepperEnv):
        """Current object position w.r.t. end-effector frame."""
        return env.object.data.root_pos_w - env.robot.data.ee_state_w[:, :3]

    #def object_relative_tool_orientations(self, env: PepperEnv):
    ...
    #def object_desired_positions(self, env: PepperEnv):
        """Desired object position."""
        return env.object_des_pose_w[:, 0:3] - env.envs_positions

    #def object_desired_orientations(self, env: PepperEnv):
    ...
    def base_actions(self, env: PepperEnv):
        """Last base actions provided to env."""
        return env.actions

    #def tool_actions(self, env: PepperEnv):
        """Last tool actions provided to env."""
        return env.actions[:, -1].unsqueeze(1)

    #def tool_actions_bool(self, env: PepperEnv):
...
class MoveRewardManager(RewardManager):
    ...
    #def penalizing_facing_direction(self, env: PepperEnv):
        #"""Penalize base facing  error using L2-kernel."""
        #err = torch.dot(env.robot.data.root_quat_w[:, 0:3],env.object.data.root_quat_w)/(torch.norm(env.robot.data.root_quat_w[:, 0:3])*torch.norm(env.object.data.root_quat_w[:, 0:3]))
        #return torch.where(err<0.02 and err>0.0, 1.0, 1e-4)

    #def penalizing_collision(self, env: PepperEnv):
        #"""Penalize base facing  error using L2-kernel."""
        #collision_err = torch.sum(torch.square(env.robot.data.root_pos_w[:, 0:3] - env.object.data.root_pos_w), dim=1)
        #return torch.where(collision_err<0.0, 1.0, 0.0)

    def reaching_position(self, env: PepperEnv):
        """Penalize base tracking position error using L2-kernel."""
        return torch.sum(torch.square(env.robot.data.base_dof_pos[:, :env.robot.base_num_dof] - env.object.data.root_pos_w[:, :3]), dim=1)

    #def reaching_object_position_exp(self, env: PepperEnv, sigma: float):
        """Penalize base tracking position error using exp-kernel."""
        error = torch.sum(torch.square(env.robot.data.root_pos_w[:, 0:3] - env.object.data.root_pos_w), dim=1)
        return torch.exp(-error / sigma)

    #def reaching_object_position_tanh(self, env: PepperEnv, sigma: float):
        """Penalize base error using tanh-kernel."""
        # distance of end-effector to the object: (num_envs,)
        ee_distance = torch.norm(env.robot.data.root_pos_w[:, 0:3] - env.object.data.root_pos_w, dim=1)
        ...
        #tool_sites_distance = torch.norm(env.robot.data.root_pos_w[:, 0:3] - object_root_pos, dim=-1)
    ...
    #def penalizing_tool_action_l2(self, env: PepperEnv):
        """Penalize large values in action commands for the tool."""
        return -torch.square(env.actions[:, -1])

    #def tracking_object_position_exp(self, env: PepperEnv, sigma: float, threshold: float):
        """Penalize base position error using exp-kernel."""
    ...
    #def tracking_object_position_tanh(self, env: PepperEnv, sigma: float, threshold: float):
    ...
    def tracking_position_success(self, env: PepperEnv, threshold: float):
        ...
        return torch.where(env.robot.data.base_dof_pos[:, 2] > threshold, 1.0, 0.0)
